...
export function AnswerSheet({ round, onResultChange }: { round: MomRound; onResultChange?: (itemId: string, result: "correct" | "incorrect" | null) => void }) {
  ...
  return <section className="panel"><h2>답지 · 채점</h2>{round.items.map((item)=><div className="answer-row" key={item.id}><b>{item.position}</b><span className="answer-glyph">{item.character.glyph}</span><span className="answer-text"><strong>{item.character.meaning} {item.character.reading}</strong><small>{results[item.id] === "correct" ? "맞음" : results[item.id] === "incorrect" ? "틀림" : "아직 채점하지 않음"}</small></span><span className="mark"><button className={results[item.id] === "correct" ? "selected-correct" : ""} onClick={()=>mark(item.id,"correct")}>맞음</button><button className={results[item.id] === "incorrect" ? "selected-incorrect" : ""} onClick={()=>mark(item.id,"incorrect")}>틀림</button></span></div>)}</section>;
}
